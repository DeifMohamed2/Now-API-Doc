...
<ParamField body="productDescription" type="string">
  Updated product description. Required for `Deliver` and `Exchange` orders.
</ParamField>

<ParamField body="numberOfItems" type="number">
  Updated item count. Required for `Deliver` and `Exchange` orders.
</ParamField>
...
<ParamField body="COD" type="boolean">
  Cash on delivery flag (`Deliver` orders).
</ParamField>
